...
align_file.join(in_var_ch).set { samp_ch }

samp_ch.into { data1; data2; data3; data4; data5 }
...
//   maxForks 10
    ...
    input:
    set val(name), file(variants) from data4

    output:
    set val(name), file("${name}_all_norm.vcf.gz"), file("${name}_all_norm.vcf.gz.tbi") into (var_norm1, var_norm2)

    script:
    """
	bcftools view -r ${region_b2} ${variants.get(2)} | bgzip -c > ${name}_${region_b2}.vcf.gz
	tabix ${name}_${region_b2}.vcf.gz
        bcftools norm -m - ${name}_${region_b2}.vcf.gz | bcftools view -e 'GT="1/0"' | bcftools view -e 'GT="0/0"' | bgzip -c > ${name}_all_norm.vcf.gz
        tabix ${name}_all_norm.vcf.gz
...
//   maxForks 10
    ...
    input:
    set val(name), file(norm_vars) from var_norm1
    path res_dir
