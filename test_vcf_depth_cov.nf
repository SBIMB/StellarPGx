...
//   maxForks 10
    ...
    input:
    set val(name), file("samp_dp2") from sv_ch3_1

    output:
    set val(name), file("${name}_gene_del_summary.txt") into del_ch

    script:

    """
    touch ${name}_gene_del_summary.txt
    """
...
fin_files2.join(sv_ch3_2).set {fin_files}
