...
def get_total_CN(cov_file):
    ...
    av_4f2_cov = float(all_reg[0][3])/(float(all_reg[0][2]) - float(all_reg[0][1]))
    ...
    comp_av = av_4f2_cov/av_ctrl_cov
    temp_cn = 2 * comp_av
    total_cn = round(temp_cn)


    return [str(int(total_cn)), round(av_4f2_cov), round(av_ctrl_cov)]; # , str(av_e1_int4), str(av_int4_e9)];
