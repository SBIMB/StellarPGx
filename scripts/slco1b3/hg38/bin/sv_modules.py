...
def get_total_CN(cov_file):
    ...
    av_slco1b3_cov = float(all_reg[0][3])/(float(all_reg[0][2]) - float(all_reg[0][1]))
    ...
    comp_av = av_slco1b3_cov/av_ctrl_cov
    temp_cn = 2 * comp_av
    total_cn = round(temp_cn)


    return [str(int(total_cn)), round(av_slco1b3_cov), round(av_ctrl_cov), exon_cov_list]; 
